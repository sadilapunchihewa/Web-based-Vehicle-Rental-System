...
<head>
    ...
    <title>Drive LK - Vehicle Details</title>
...
<div class="d-flex flex-column min-vh-100">
    ...
    <div class="custom-container custom-padding-x py-5">
        <h2 class="fs-2 fw-bold mb-5">Vehicle Details</h2>
        <div class="card border-0 shadow-sm">
            <img src="${vehicle.imageBase64}" class="vehicle-image" alt="${vehicle.brand} ${vehicle.model}">
            <div class="card-body">
                <h3 class="card-title text-dark-gray mb-4">${vehicle.brand} ${vehicle.model}</h3>
                <div class="row">
                    <div class="col-md-6">
                        <p class="text-muted-gray"><strong>Vehicle ID:</strong> ${vehicle.id}</p>
                        <p class="text-muted-gray"><strong>Type:</strong> ${vehicle.type}</p>
                        <p class="text-muted-gray"><strong>Year:</strong> ${vehicle.year}</p>
                        <p class="text-muted-gray"><strong>Mileage:</strong> ${vehicle.mileage} km</p>
                        <p class="text-muted-gray"><strong>Price Per Day:</strong> ${vehicle.pricing}$</p>
                        <p class="text-muted-gray"><strong>Fuel Type:</strong> ${vehicle.fuelType}</p>
                    </div>
                    <div class="col-md-6">
                        <p class="text-muted-gray"><strong>Seating Capacity:</strong> ${vehicle.seatingCapacity}</p>
                        <p class="text-muted-gray"><strong>Status:</strong> ${vehicle.status}</p>
                        <p class="text-muted-gray"><strong>Description:</strong> ${vehicle.description}</p>
                    </div>
                </div>
                <c:if test="${not empty vehicle.features}">
                    <h5 class="mt-4 text-dark-gray">Features</h5>
                    <ul class="feature-list">
                        <c:forEach var="feature" items="${vehicleFeatures}">
                            <li class="text-muted-gray">${feature}</li>
                        </c:forEach>
                    </ul>
                </c:if>
                <div class="d-flex gap-2 flex-wrap mt-4">
                    <a href="/vehicle_edit/${vehicle.id}" class="btn btn-outline-primary">Edit</a>
                    <c:if test="${vehicle.status == 'Available'}">
                        <a href="/vehicle_rent/${vehicle.id}" class="btn btn-outline-success">Rent Now</a>
                    </c:if>
                    <c:if test="${vehicle.status != 'Pending Deletion'}">
                        <a href="/vehicle_delete/${vehicle.id}" class="btn btn-outline-danger">Request Delete</a>
                    </c:if>
                    <c:if test="${vehicle.status == 'Pending Deletion'}">
                        <a href="/vehicle_confirm_delete/${vehicle.id}" class="btn btn-outline-warning">Confirm Delete (Admin)</a>
                    </c:if>
                    <a href="/vehicle_list" class="btn btn-outline-secondary">Back to List</a>
                </div>
            </div>
        </div>
    </div>
